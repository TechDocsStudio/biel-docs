...
| `think-mode-enabled`   | **Type**: `boolean`<br />**Default value**: `false`<br /><br />When set to `true`, enables Think Mode by default for deeper AI reasoning. Requires the reasoning mode plugin to be enabled for your project. See [Think Mode](./think-mode) for details. |
| **Text**             |  |  |
