...
For example, you can use it to embedded Biel.ai at the end of every page as follows:
...
1. For Docusaurus, the best approach is to swizzle the original doc footer component to embed the Biel.ai button. Begin by using the npm run swizzle command to start the swizzle process:
...
1. After swizzling, navigate to `src/theme/DocItem/Footer/index.js` in your project directory. Now, integrate the Biel.ai button:
...
1. For Docusaurus, the best approach is to swizzle the original footer component to embed the Biel.ai button. This ensures the feedback button is loaded just before the closing body tag for optimal performance. Begin by using the npm run swizzle command to start the swizzle process:
...
1. After swizzling, navigate to `src/theme/Footer/index.js` in your project directory. Replace the content with the following to integrate the Biel.ai button:
    ...
    function FeedbackWidget() {
        ...
        const placeholders = {
            en: {
                ...
                modalTitleError403: "The request URL does not match the one defined in Biel.ai for this project.",
                modalTitleError404: "We could not find the provided project id in Biel.ai.",
            ...
            es: {
                ...
                modalTitleError403: "La URL de solicitud no coincide con la definida en Biel.ai para este proyecto.",
                modalTitleError404: "No pudimos encontrar el ID del proyecto proporcionado en Biel.ai.",
