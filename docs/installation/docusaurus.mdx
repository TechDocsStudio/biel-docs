...
## Advanced setup: Biel Search

To integrate the Biel Search widget into your Docusaurus site:
...
2. With your terminal still open, run the following command to install Biel Search:
...
3. For Docusaurus, the best approach is to swizzle the original doc SearchBar component to embed the Biel Search button. Begin by using the npm run swizzle command to start the swizzle process:
...
5. Now you’ll need to update the `tsconfig.json` file to ensure compatibility with React components. Open `tsconfig.json` in your project root directory and add the following options to `compilerOptions`:
    ```json
    {
        "compilerOptions": {
